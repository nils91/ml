...
def get_maximum_combinational_probability_init(i):
    return start_state_prob[i-1]*get_emissions_prob_in_state(i,get_o(1))
...
def get_maximum_combinational_probability_rec(t,i):
    ...
    for j in range(1,len(states)+1):
        maximum=max(maximum,get_trans_prob(j,i)*get_maximum_combinational_probability_rec(t-1,j))
    return get_emissions_prob_in_state(i,get_o(t))*maximum
